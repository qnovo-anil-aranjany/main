...
if not SKIP_TEST:
    ...
    def parse_AFC_processed_Behavioral_Test(file_path):
        test_cases = []
        for row in iter_file(file_path):
            ...
            SEVolts_DR = [int(item) for item in ast.literal_eval(row["SEVolts_DR"])]

            TempSnsrs = [int(item) for item in ast.literal_eval(row["TempSnsrs"])]

            TempSnsrs_DR = [int(item) for item in ast.literal_eval(row["TempSnsrs_DR"])]
            ...
            logged_VeAFC_b_Initialized = max(
                0,
                int(row["QnovoAFC_LogVar2\nl_InitializedFlag"]) - CeAPI_b_LogOffsetZero,
            )
            ...
            logged_VeAFC_Cnt_PresentStgNum = max(
                0,
                int(row["QnovoAFC_LogVar5\nl_PresentStageNum"]) - CeAFC_cmp_LogOffset,
            )
            ...
            logged_VaAFC_b_ValidSampleFlag = [
                ...
                for item in ast.literal_eval(row["QnovoAFC_LogVar3\nl_ValidSampleFlag"])
            ]

            logged_VaAFC_U_SampleSEVolt = [
                max(0, (int(item) - CeAFC_cmp_LogOffsetZero))
                for item in ast.literal_eval(row["QnovoAFC_LogVar15\nl_SampleSEVolt"])
            ]
            ...
            logged_VaAFC_U_RefSEVolt = [
                max(0, (int(item) - CeAFC_cmp_LogOffsetZero))
                for item in ast.literal_eval(row["QnovoAFC_LogVar16\nl_RefSEVolt"])
            ]
            ...
            test_case = {
                "Inputs": {
                    ...
                    "logged_VaAFC_U_SampleSEVolt": logged_VaAFC_U_SampleSEVolt,
                    "logged_VaAFC_Cnt_CPVCorrIdx": logged_VaAFC_Cnt_CPVCorrIdx,
                    "logged_VaAFC_U_RefSEVolt": logged_VaAFC_U_RefSEVolt,
                },
                "Expected": {},
            }
    ...
    def test_AFC_processed_Behavioral_log_validate(lib: Any):
        ...
        all_time_steps = parse_AFC_processed_Behavioral_Test(file_path)
        ...
        for each_time_step in all_time_steps:
            ...
            if input_time == start_time - 1:
                lib.sAFC_Calc.Ve_I_CV_Curr = each_time_step["Inputs"][
                    "logged_VeAFC_I_CV_Curr"
                ]
                lib.sAFC_Calc.Va_b_ValidSampleFlag = each_time_step["Inputs"][
                    "logged_VaAFC_b_ValidSampleFlag"
                ]
                lib.sAFC_Calc.Va_U_SampleSEVolt = each_time_step["Inputs"][
                    "logged_VaAFC_U_SampleSEVolt"
                ]
            ...
            if input_time > start_time - 1:
                ...
                if input_time == start_time:
                    lib.sAFC_Calc.Ve_e_QNS_State = each_time_step["Inputs"][
                        "logged_VeAFC_e_QNS_State"
                    ]
                    lib.sAFC_Calc.Ve_Cnt_PresentStgNum = each_time_step["Inputs"][
                        "logged_VeAFC_Cnt_PresentStgNum"
                    ]

                # these could be updated at any time step due to LoadNVM and SaveNVM
                lib.sAFC_Calc.Va_Cnt_CPVCorrIdx = each_time_step["Inputs"][
                    "logged_VaAFC_Cnt_CPVCorrIdx"
                ]
                lib.sAFC_Calc.Va_U_RefSEVolt = each_time_step["Inputs"][
                    "logged_VaAFC_U_RefSEVolt"
                ]
                ...
                results["SEVolts_DR"].append(
                    lib_array_to_list(lib.VaAPI_b_SEVolts_DR)
                )
        ...
        res_data = parse_AFC_processed_Behavioral_Test(res_path)
        ref_data = parse_AFC_processed_Behavioral_Test(ref_path)
